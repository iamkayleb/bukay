...
def _extract_balanced_block(text: str, opening_brace_index: int) -> str:
    ...
    raise AssertionError("unterminated object block")
...
def _compound_key_block(where_clause: str, compound_key: str) -> str:
    match = re.search(rf"\b{compound_key}:\s*\{{", where_clause)
    assert match, f"where clause must contain {compound_key}"
    return _extract_balanced_block(where_clause, match.end() - 1)
...
def test_tenant_scoped_upserts_use_only_compound_unique_key_in_where() -> None:
    ...
    for model, compound_key in expected_compound_keys.items():
        ...
        assert not re.search(
            ...
            where_clause.replace(_compound_key_block(where_clause, compound_key), ""),
            re.DOTALL,

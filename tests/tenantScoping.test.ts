...
const isPostgresDatabase = /^postgres(?:ql)?:\/\//.test(databaseUrl ?? "");
const describeIfPostgres = isPostgresDatabase ? describe : describe.skip;
...
describeIfPostgres("tenant-scoped Prisma queries", () => {
  beforeAll(async () => {
    await resetDatabase();

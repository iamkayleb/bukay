...
def _all_migration_sql() -> str:
    sql_files = sorted(MIGRATIONS_DIR.glob("*/migration.sql"))
    assert sql_files, f"no migration SQL files found under {MIGRATIONS_DIR}"
...
def test_prisma_migrate_dev_runs_on_clean_database(tmp_path: Path) -> None:
    ...
    if "DATABASE_URL" not in os.environ:
        pytest.skip("DATABASE_URL is required for live Prisma migrate checks")

...
ROUND="${1:?usage: reseed-round.sh ROUND [--apply] [--agent NAME]}"
APPLY=0; ONLY=""
shift || true
while [ $# -gt 0 ]; do
  case "$1" in
...
if [ -n "$ONLY" ]; then
  mapfile -t NUMS < <(gh issue list --repo "$REPO" --label "eval:round-$ROUND" --state all \
    ...
    -q ".[] | select([.labels[].name] | index(\"agent:$ONLY\")) | .number" | sort -n)
else
  mapfile -t NUMS < <(gh issue list --repo "$REPO" --label "eval:round-$ROUND" --state all \
...
if [ "${#NUMS[@]}" -eq 0 ]; then
  ...
  for n in "${NUMS[@]}"; do
    for b in $(gh api "repos/$REPO/branches?per_page=100" -q '.[].name' 2>/dev/null \
               | grep -E "^${ONLY:-(claude|codex|cursor)}/issue-$n$" || true); do
...
if [ "$APPLY" -eq 1 ]; then
  ...
  echo "  ./eval/create-round.sh $ROUND${ONLY:+ --agent $ONLY}            # dry run"
  echo "  ./eval/create-round.sh $ROUND --apply${ONLY:+ --agent $ONLY}"
else

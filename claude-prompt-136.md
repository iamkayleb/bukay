...
**Progress:** 1/7 tasks complete, 6 remaining

### Scope
Owner can add a booking from the dashboard: pick or create a client, select service, select slot, add notes. Enforce no-double-book via a DB constraint.

### Tasks
Complete these in order. Mark checkbox done ONLY after implementation is verified:

- [x] Add Postgres exclusion constraint on (staffId, tstzrange) for overlaps
- [ ] Booking form with client search and inline create
...
- [ ] Overlapping booking on same staff rejected at DB
- [ ] Created booking appears in calendar immediately
- [ ] Audit log shows manual_booking_created

### Suggested Next Task
- Add Postgres exclusion constraint on (staffId, tstzrange) for overlaps

...
`SlotHold` temporarily reserves a booking slot for a session until `expiresAt`. `slotKey` is globally
unique, preventing concurrent sessions from holding the same slot; expired holds can be located via
the expiry index.

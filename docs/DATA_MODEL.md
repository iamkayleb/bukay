...
| `SlotHold` | Temporary reservation for a bookable slot | `@unique(slotKey)`, `@@index([tenantId])`, `@@index([expiresAt])` |
...
`SlotHold` temporarily reserves a tenant's bookable slot while a customer completes the booking flow.
`slotKey` is globally unique, `sessionId` identifies the holding session, and `expiresAt` lets stale
holds be removed safely.

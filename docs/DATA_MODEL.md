...
| `SlotHold` | Temporary reservation preventing concurrent booking of a slot | `@unique(slotKey)`, `@@index([tenantId])`, `@@index([expiresAt])` |
...
`SlotHold` temporarily reserves a tenant's booking slot for one session. `slotKey` is globally unique,
which prevents more than one active hold for the same slot; `expiresAt` is indexed so expired holds can
be efficiently cleaned up.

...
describe("getAvailabilityWindows", () => {
  ...
  it("uses getOpenWindows to load availability for the requested date", () => {
  ...
  it("returns no availability when getOpenWindows reports a blackout", () => {
    calls.getOpenWindows.mockReturnValue([]);

    expect(getAvailabilityWindows({ startDate: "2026-12-25" }, config)).toEqual([]);
  });

  it("normalizes Date inputs before loading availability", () => {

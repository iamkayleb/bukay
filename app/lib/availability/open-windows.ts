...
// Structural equivalent for a Blackout row. Presence of a matching row for
...
// in `timezone`. Uses Intl.DateTimeFormat so callers can pass any IANA zone;
// for the tenant-default "Africa/Lagos" (no DST) day boundaries line up
// cleanly, which is the guarantee this helper relies on.
...
// Pure availability core. Blackouts short-circuit to an empty list; rows
// flagged `isClosed` are ignored; and rows with `closesAt <= opensAt` are
// dropped (guards against operator typos rather than trying to model
// overnight windows, which the weekly-schedule task explicitly does not
// cover). Multiple windows per weekday are supported — the output is sorted
// by opensAt so downstream consumers can iterate deterministically.
export function computeOpenWindows(input: ComputeOpenWindowsInput): OpenWindow[] {
  ...
  if (blackouts.some((b) => normalizeIsoDate(b.date) === isoDate)) {
    return [];
  }
...
// Structural Prisma surface the helper needs. Tests pass a stub with the same
// shape instead of the live client.

...
export async function POST(request: NextRequest) {
  ...
  const booking = await transactionDelegate.$transaction(async (transaction) => {
    const holds = new SlotHoldStore(transaction.slotHold);
    if (!(await holds.acquire(slot, service.tenantId, parsed.data.sessionId))) {
      return null;
    }

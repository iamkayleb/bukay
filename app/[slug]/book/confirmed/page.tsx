...
export default async function BookingConfirmedPage({ params, searchParams }: ConfirmedPageProps) {
  ...
  const rescheduleHref = `/${booking.tenantSlug}/book?action=reschedule&token=${encodeURIComponent(token)}`;
  const cancelHref = `/${booking.tenantSlug}/book?action=cancel&token=${encodeURIComponent(token)}`;
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col gap-6 px-6 py-16 text-slate-100">
      ...
      <nav aria-label="Booking actions" className="flex flex-wrap gap-4 text-sm">
        <a href={rescheduleHref} className="font-medium text-emerald-300 hover:text-emerald-200">
          Reschedule
        </a>
        <a href={cancelHref} className="font-medium text-slate-400 hover:text-slate-200">
          Cancel booking
        </a>
      </nav>
    </main>

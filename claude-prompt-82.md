...
**Progress:** 1/10 tasks complete, 9 remaining

### Scope
Implement phone-number authentication with OTP via an SMS provider port (Termii or equivalent adapter). Normalize Nigerian numbers to E.164 (+234). Use signed HTTP-only session cookies.
...
- [x] User can sign up and log in by phone + OTP
- [x] Expired/used OTP rejected
- [x] Rate limit blocks brute force
- [x] Session persists across reloads

### Recently Attempted Tasks
Avoid repeating these unless a task needs explicit follow-up:

- Add SmsProvider interface + Termii adapter
